...
#pragma omp parallel for num_threads(thread_num)
  ...
  inline void parse_args(int argc, char **argv) {
    ...
    operations_num =
        stoi(get_with_default(flags, "operations_num", "1000000000"));
    ...
    std::string order_str =
        get_with_default(flags, "operation_order", "iterate");
    if (order_str == "iterate") {
      ...
      COUT_THIS("Unknown operation order: " << order_str
                                            << ". Using default (iterate).");
    }
  ...
  void
  generate_operations(KEY_TYPE *keys,
    ...
    // Pre-generate sample keys for reads/updates/scans using surviving
    // active_keys
    KEY_TYPE *sample_ptr = nullptr;
    if (!active_keys.empty()) {
      if (sample_distribution == "uniform") {
        sample_ptr = get_search_keys(active_keys.data(), active_keys.size(),
                                     total_ops, &random_seed);
      } else if (sample_distribution == "zipf") {
        sample_ptr = get_search_keys_zipf(
            active_keys.data(), active_keys.size(), total_ops, &random_seed);
      }
    }

    // Generate reads, updates, and scans
    size_t sample_counter = 0;
    for (size_t i = 0; i < read_count + update_count + scan_count; i++) {
      if (active_keys.empty() || sample_ptr == nullptr)
        continue;

      KEY_TYPE key = sample_ptr[sample_counter++];
    ...
    for (size_t i = 0; i < insert_count; i++) {
      if (next_insert_pos >= table_size)
        continue;
      KEY_TYPE key = keys[next_insert_pos++];
      temp_operations.emplace_back(INSERT, key);
      active_keys.push_back(
          key); // Add to active keys so future runs can see it
    }
    ...
    case ITERATE_SORT_UNSORTED:
      if (current_batch_sorted) {
        std::sort(temp_operations.begin(), temp_operations.end(),
                  operation_comp);
      } else {
    ...
    std::string filename = std::string("operations_") + current_index_name +
                           "_" + std::to_string(iteration_num) + ".txt";
    ...
    if (!is_first_call) {
      init_keys.clear();
      init_keys.resize(active_keys.size());
      for (size_t i = 0; i < active_keys.size(); ++i) {
        init_keys[i] = active_keys[i];
      }
      tbb::parallel_sort(init_keys.begin(), init_keys.end());

      if (init_key_values) {
        delete[] init_key_values; // avoid mem leak
      }
      init_key_values = new std::pair<KEY_TYPE, PAYLOAD_TYPE>[init_keys.size()];
  ...
  void update_init_keys_and_values_threaded(KEY_TYPE *keys,
                                            size_t init_table_size,
                                            int thread_num) {
...
#pragma omp parallel num_threads(thread_num)
    {
      ...
      thread_param.read_latency.reserve(operations_num /
                                        latency_sample_interval);
      thread_param.write_latency.reserve(operations_num /
                                         latency_sample_interval);
      PAYLOAD_TYPE val;
      ...
      for (auto i = 0; i < operations_num; i++) {
        ...
        if (op == READ) {
          ...
          if (latency_sample && i % latency_sample_interval == 0) {
            ...
            thread_param.read_latency.push_back(std::make_pair(
          ...
          if (latency_sample && i % latency_sample_interval == 0) {
            latency_sample_end_time = tn.rdtsc();
            thread_param.write_latency.push_back(std::make_pair(
                latency_sample_start_time, latency_sample_end_time));
          }
        } else if (op == UPDATE) {
...
#pragma omp master
    ...
    for (auto &p : params) {
      if (latency_sample) {
        ...
        for (auto e : p.write_latency) {
          stat.write_latency.push_back(tn.tsc2ns(e.second) -
                                       tn.tsc2ns(e.first));
        }
      }
    ...
    stat.throughput =
        static_cast<uint64_t>(operations.size() / (diff / (double)1000000000));

    if (dataset_statistic) {
      tbb::parallel_sort(init_keys.begin(), init_keys.end());
      stat.fitness_of_dataset = pgmMetric::PGM_metric(
  ...
  void print_stat(bool header = false, bool clear_flag = true) {
    ...
    if (latency_sample) {
      if (!stat.read_latency.empty()) {
        for (auto t : stat.read_latency) {
          read_latency_variance +=
              (t - avg_read_latency) * (t - avg_read_latency);
        }
      ...
      if (!stat.write_latency.empty()) {
        for (auto t : stat.write_latency) {
          write_latency_variance +=
              (t - avg_write_latency) * (t - avg_write_latency);
        }
    ...
    if (latency_sample) {
      ...
      ofile << (stat.read_latency.empty()
                    ? 0
                    : stat.read_latency[0.5 * stat.read_latency.size()])
            << ",";
      ofile << (stat.read_latency.empty()
                    ? 0
                    : stat.read_latency[0.9 * stat.read_latency.size()])
            << ",";
      ofile << (stat.read_latency.empty()
                    ? 0
                    : stat.read_latency[0.99 * stat.read_latency.size()])
            << ",";
      ofile << (stat.read_latency.empty()
                    ? 0
                    : stat.read_latency[0.999 * stat.read_latency.size()])
            << ",";
      ofile << (stat.read_latency.empty()
                    ? 0
                    : stat.read_latency[0.9999 * stat.read_latency.size()])
            << ",";
      ofile << (stat.read_latency.empty()
                    ? 0
                    : stat.read_latency[stat.read_latency.size() - 1])
            << ",";
      ofile << (stat.read_latency.empty() ? 0 : avg_read_latency) << ",";
      ofile << (stat.write_latency.empty() ? 0 : stat.write_latency[0]) << ",";
      ofile << (stat.write_latency.empty()
                    ? 0
                    : stat.write_latency[0.5 * stat.write_latency.size()])
            << ",";
      ofile << (stat.write_latency.empty()
                    ? 0
                    : stat.write_latency[0.9 * stat.write_latency.size()])
            << ",";
      ofile << (stat.write_latency.empty()
                    ? 0
                    : stat.write_latency[0.99 * stat.write_latency.size()])
            << ",";
      ofile << (stat.write_latency.empty()
                    ? 0
                    : stat.write_latency[0.999 * stat.write_latency.size()])
            << ",";
      ofile << (stat.write_latency.empty()
                    ? 0
                    : stat.write_latency[0.9999 * stat.write_latency.size()])
            << ",";
      ofile << (stat.write_latency.empty()
                    ? 0
                    : stat.write_latency[stat.write_latency.size() - 1])
            << ",";
  ...
  void run_benchmark() {
    ...
    std::vector<std::pair<Operation, KEY_TYPE>> operations;
    int n_runs = (table_size - (table_size * init_table_ratio)) /
