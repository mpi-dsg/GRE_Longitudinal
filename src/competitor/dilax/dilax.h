...
class dilaxInterface : public indexInterface<KeyType, PayloadType> {
public:
  void init(Param *param = nullptr) {
    // Set mirror directory for DILAX
    index.set_mirror_dir("/tmp/dilax_mirror");
  }
...
template<class KeyType, class PayloadType>
void dilaxInterface<KeyType, PayloadType>::bulk_load(std::pair <KeyType, PayloadType> *key_value, size_t num,
                                                      Param *param) {
  // Convert to the format expected by DILAX (long, long pairs)
...
template<class KeyType, class PayloadType>
bool dilaxInterface<KeyType, PayloadType>::get(KeyType key, PayloadType &val, Param *param) {
  long result = index.search(static_cast<long>(key));
  if (result >= 0) {  // DILAX returns -1 for not found
...
template<class KeyType, class PayloadType>
bool dilaxInterface<KeyType, PayloadType>::put(KeyType key, PayloadType value, Param *param) {
  return index.insert(static_cast<long>(key), static_cast<long>(value));
}

template<class KeyType, class PayloadType>
bool dilaxInterface<KeyType, PayloadType>::update(KeyType key, PayloadType value, Param *param) {
  // DILAX doesn't have direct update, so do delete + insert
...
template<class KeyType, class PayloadType>
bool dilaxInterface<KeyType, PayloadType>::remove(KeyType key, Param *param) {
  recordPtr deleted_ptr = index.delete_key(static_cast<long>(key));
  return deleted_ptr >= 0;
}

template<class KeyType, class PayloadType>
size_t dilaxInterface<KeyType, PayloadType>::scan(KeyType key_low_bound, size_t key_num,
                                                   std::pair<KeyType, PayloadType> *result,
                                                   Param *param) {
  // DILAX range_query signature: range_query(k1, k2, recordPtr *ptrs)
  ...
  for (size_t i = 0; i < scan_size; i++) {
    result[i] = {static_cast<KeyType>(k1 + i), static_cast<PayloadType>(ptrs[i])}; // Approximate - DILAX doesn't return keys directly
  }
