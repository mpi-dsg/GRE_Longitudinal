...
private:
  kanva::Kanva_RS<KEY_TYPE, PAYLOAD_TYPE> index;
};
...
bool kanvaInterface<KEY_TYPE, PAYLOAD_TYPE>::get(KEY_TYPE key, PAYLOAD_TYPE &val, Param *param) {
  kanva::result_t res = index.find(key, val);
  if(res == kanva::result_t::ok) {
    return true;
  }
  return false;
}
...
bool kanvaInterface<KEY_TYPE, PAYLOAD_TYPE>::put(KEY_TYPE key, PAYLOAD_TYPE value, Param *param) {
  kanva::result_t res = index.insert(key, value);
  if(res == kanva::result_t::ok) {
    return true;
  }
  return false;
...
bool kanvaInterface<KEY_TYPE, PAYLOAD_TYPE>::remove(KEY_TYPE key, Param *param) {
 kanva::result_t res = index.remove(key);
  if(res == kanva::result_t::ok) {
    return true;
  }
  return false;
