#!/bin/bash
# Growth-only longitudinal ablation: alex vs alex_rls variants on libio.
set -euo pipefail
cd /home/GRE_Longitudinal

OUT_DIR=./results/exploratory/alex_rls_ablation
...
INDEXES=(alex alex_rls alex_rls_off alex_rls_nofs alex_rls_batch128)

# Fresh CSV for this suite
: > "$CSV"

for idx in "${INDEXES[@]}"; do
  log="${LOG_DIR}/alex_rls_ablation_${idx}_libio_5m_seed${SEED}.log"
  {
    echo ""
    echo "======================================================================"
...
echo "All ablations finished. CSV=${CSV}"
